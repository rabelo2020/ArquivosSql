...
use exercicio;
SELECT nome, sexo, email, salario FROM funcionarios limit 10;
 SELECT nome, sexo, email, salario FROM funcionarios WHERE salario between 76902 and 108659;
 SELECT nome, sexo, salario, cargo, admissao FROM funcionarios WHERE salario IN(SELECT MIN(salario) FROM funcionarios);
 
 UPDATE funcionarios SET nome='Arthur' WHERE nome='Miller' AND idFuncionario=785;
 select idFuncionario, nome, sexo, admissao from funcionarios where nome='Arthur';
 SELECT departamento,COUNT(departamento)AS Qtd FROM funcionarios GROUP BY departamento ORDER BY 2;
 SELECT idRegiao,COUNT(idRegiao)AS Qtd FROM funcionarios GROUP BY idRegiao ORDER BY 1;
